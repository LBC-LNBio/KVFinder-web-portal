...
create_init_scene <- function(input, output, result_pdb_list, is_pg2, scheme_color_list) {
  ...
  output[[protein_color_scheme]] <- renderUI({
    div(
      style = "font-size:12px;",
      selectInput(
        inputId = paste("input_", protein_color_scheme, sep = ""), label = div(style = "font-size:12px", "Protein color scheme"),
        choices = names(scheme_color_list)
      )
    )
  })
  # protein color selector
  output[[protein_color]] <- renderUI({
    div(
      style = "font-size:12px;",
  ...
  output[[cavity_color]] <- renderUI({
    div(
      style = "font-size:12px;",
      selectInput(
        inputId = paste("input_", cavity_color, sep = ""), label = div(style = "font-size:12px", "Cavity color"),
        choices = c("white", "red", "blue", "green", "yellow")
      )
    )
  })
  # protein representation selector
  output[[protein_rep]] <- renderUI({
    div(
      style = "font-size:12px;",
      selectInput(
  ...
  output[[snapshot_title]] <- renderUI({
    div(style = "font-size:12px;font-weight: bold", "Snapshot")
  })
  output[[snapshot]] <- renderUI({
    div(
      style = "font-size:12px;",
      actionButton(
        inputId = paste("input_", snapshot, sep = ""), label = "",
        width = 40, icon = icon("camera")
      )
    )
  })
  # fullscreen button
  output[[fullscreen_title]] <- renderUI({
    div(style = "font-size:12px;font-weight: bold", "Fullscreen")
  })
  output[[fullscreen]] <- renderUI({
    div(
      style = "font-size:12px;",
      actionButton(
