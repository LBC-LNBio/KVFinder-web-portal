...
process_fetch <- function(input, output) {
  ...
  if (length(get_nonstand) > 0) {
    ...
    output$checkbox_nostand2 <- renderUI({
      checkboxGroupInput(inputId = "select_nonstand2", label = "Non-standard residues found (select to consider them as part of the biomolecular surface in the cavity detection):", choices = get_nonstand, selected = TRUE)
    })
