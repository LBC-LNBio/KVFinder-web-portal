...
kv_help <- function(){
  ...
  fluidRow(
    column(12, align="center", tags$h5(strong("Main impact of parameters on the cavity detection:"))
    ),
    
    tags$li(
      strong("Probe out:"), 
      "the Probe Out translates the biomolecular structure to create a molecular surface, producing the boundary between the cavity and the bulk due to the restricted access to the empty space within the protein. Thus, greater Probe Out sizes tend to reduce the degree of accessibility of the molecular surface created and ultimately, increasing the elapsed time to perform calculations in KVFinder-web service."
      ),
    tags$li(
      strong("Removal Distance:"),
      "the Removal Distance removes cavity points within a given length from the defined cavity-bulk boundary. Thus, reducing the Removal Distance parameter removes less points from the boundary, which helps to segregate sub-pockets and/or detect superficial cavities."
      ),
  ),
  ...
  fluidRow(
    ...
    tags$p(
      "The KVFinder-web service has a Job timeout of 30 minutes (maximum time that a accepted job could run on the server), completed jobs will be available on the web service up to 1 day after completion, and the maximum payload (maximum size of the JSON) of the data sent to the KVFinder-web service is 1 MB.",
      style= "text-align:justify"
      ),
    tags$p(
