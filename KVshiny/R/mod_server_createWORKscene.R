...
create_work_scene <- function(input, output, protein_rep_list, protein_col_list, result_pdb_list, is_pg2){
  if(is_pg2 == TRUE){
    ...
    NGLVieweR_proxy(structure) %>%
      addSelection(tail(protein_rep_list, n=1),
                   param =
                     list(
                       name = "sel2",
                       sele = "protein", #this need to be changed to include also nucleic acid 
                       colorScheme = tail(protein_col_list ,n = 1)
                     ))
