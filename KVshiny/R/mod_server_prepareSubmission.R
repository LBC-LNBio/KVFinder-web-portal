...
submit_prepare <- function(pdb_path, ligand_path, whole_protein_mode, ligand_mode, box_mode, box_residues, probe_in, probe_out, volume_cutoff, removal_distance, padding, lig_cutoff) {
  ...
  list_modes <- list(
    whole_protein_mode = whole_protein_mode,
    box_mode = box_mode,
    resolution_mode = "Low",
    surface_mode = TRUE,
    kvp_mode = FALSE,
  ...
  list_visiblebox <- list(
    p1 = list(x = 0.00, y = 0.00, z = 0.00),
    p2 = list(x = 0.00, y = 0.00, z = 0.00),
    p3 = list(x = 0.00, y = 0.00, z = 0.00),
    p4 = list(x = 0.00, y = 0.00, z = 0.00)
  )
  list_internalbox <- list(
    p1 = list(x = -4.00, y = -4.00, z = -4.00),
  ...
  if (ligand_mode == TRUE) { # ligand mode
    ...
    list_visiblebox <- list(
      p1 = list(x = coord_list$xmin, y = coord_list$ymin, z = coord_list$zmin),
    ...
    list_internalbox <- list(
      p1 = list(x = coord_list$xmin - probe_out, y = coord_list$ymin - probe_out, z = coord_list$zmin - probe_out),
